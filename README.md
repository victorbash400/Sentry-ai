...
Wildlife conservation risk assessment platform powered by satellite imagery and machine learning.

## Overview

Sentry is a geospatial analytics tool designed to help conservation teams identify and prioritize high-risk areas for wildlife threats such as poaching, illegal logging, and habitat encroachment. The platform combines real-time satellite data from Google Earth Engine with predictive risk modeling to provide actionable insights for wildlife protection efforts.

## Key Features

- **Interactive Map Interface**: Draw custom analysis regions or select from predefined protected areas
- **Satellite Data Integration**: Automated extraction of Sentinel-2 imagery and environmental indicators (NDVI, land cover, terrain)
- **Risk Assessment Model**: Machine learning-based threat prediction across multiple risk categories
...
- **Temporal Analysis**: Time-series risk evaluation across dawn, day, dusk, and night periods
...
- **Custom Risk Model** - Threat assessment algorithm (v1.0-simulation)
...
1. **Select Location**: Choose a protected area from the dropdown or draw a custom polygon on the map
2. **Configure Parameters**: Set date range, time of day filters, species focus, and threat types
3. **Adjust Options**: Fine-tune grid granularity, display threshold, and enabled map layers
4. **Run Analysis**: Click "Run Analysis" to start the risk assessment
...
- Sentinel-2 mission for high-resolution optical imagery
